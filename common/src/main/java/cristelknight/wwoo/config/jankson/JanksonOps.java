...
public record JanksonOps(boolean compressed) implements DynamicOps<JsonElement> {
    ...
    public DataResult<MapLike<JsonElement>> getMap(final JsonElement input) {
        ...
        return DataResult.success(new CommentsMap<>() {
            @Nullable
            @Override
    ...
    private class JsonRecordBuilder extends RecordBuilder.AbstractStringBuilder<JsonElement, JsonObject> implements Comments {
